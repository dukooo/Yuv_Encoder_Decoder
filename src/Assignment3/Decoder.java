...
public class Decoder {
	...
	String mvFileName, qtcFileName, vbsFileName, qpFileName, decodedFileName, scFileName;
	byte[][] reference; // reference[frame index][pixel index]
	short[] mvDiff;
	short[] qtc;
	byte[] vbsBytes;
	int[] qp; int RCflag; int qpFileIndex = 0;
	int[] sc;
	...
	public Decoder() {
		...
		qpFileName = ph.getProperty("qpEntropyDecodedFileName");
		scFileName = ph.getProperty("scEntropyDecodedFileName");
	...
	public void setPredictor(byte[] decoded) {
		reference = new byte[numRef][width * height];
		reference[0] = decoded;
	...
	public void decodeMV_QTC_VBS() {
		...
		if (RCflag != 0) qp = Helper.byteArrToIntArr(FileHelper.readByteFile(outputDir + qpFileName));
		if (RCflag == 2 || RCflag == 3) sc = Helper.byteArrToIntArr(FileHelper.readByteFile(outputDir + scFileName));
	}
	...
	public byte[] decodeInter() {
		...
		for (int y = 0; y < height; y+=blockSize) {
			if (RCflag != 0) {
				resApp.qp = qp[qpFileIndex++]; // reads qp value from int array of qp entropy decoded file
				// System.out.println("---> At P Frame #" + frameIndex + " Row #" + y/blockSize + ", QP = " +resApp.qp);
			}
	...
	public byte[] decodeIntra() {
		...
		for (int y = 0; y < height; y += blockSize) {
			if (RCflag != 0) {
				resApp.qp = qp[qpFileIndex++]; // reads qp value from int array of qp entropy decoded file
				// System.out.println("---> At I Frame #" + frameIndex + " Row #" + y/blockSize + ", QP = " +resApp.qp);
			}
			for (int x = 0; x < width; x+= blockSize) {
				int mode;
				if (VariableBlockSize.isVBSEnabled()) {
