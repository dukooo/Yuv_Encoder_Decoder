...
public class EntropyEncDec {
	 ...
	 * @return int, bit count of encoded content
	 */
	public static int expGolombEncBitCount(int[] a) {
		StringBuilder sb = new StringBuilder();
...
//				System.out.println(temp.get(temp.size()-1));
	...
	/**
	 * Called by encoder when all encoding is done. Assumes that all input files are ready.
	 */
	public static void generateAllEntropyEncodedFiles() {
		PropertyHelper ph = new PropertyHelper();
		String mvFileName = ph.getProperty("mvFileName");
		String vbsFileName = ph.getProperty("vbsFileName");
		String vbsEntropyEncodedFileName = ph.getProperty("vbsEntropyEncodedFileName");
		String mvEntropyEncodedFileName = ph.getProperty("mvEntropyEncodedFileName");
		String qtcDiagOrderedFileName = ph.getProperty("qtcDiagOrderedFileName");
		String qtcEntropyEncodedFileName = ph.getProperty("qtcEntropyEncodedFileName");
		String qpFileName = ph.getProperty("qpFileName");
		String qpEntropyEncodedFileName = ph.getProperty("qpEntropyEncodedFileName");
		String scFileName = ph.getProperty("scFileName");
		String scEntropyEncodedFileName = ph.getProperty("scEntropyEncodedFileName");

		// Everything is done. Entropy encode
		System.out.println("Entropy Encoding MV");
		byte[] mv = EntropyEncDec.expGolombEnc(Helper.shortArrToIntArr(FileHelper.readShortFile(System.getProperty("user.dir") + "/output/" + mvFileName)));
		FileHelper.writeToFile(mv, mvEntropyEncodedFileName);
	
		// for QTC, we need to do 2 more steps, RLE and exp-golomb
		System.out.println("RLE encode QTC");
		int[] qtcRle = EntropyEncDec.runLevelEnc(Helper.shortArrToIntArr(FileHelper.readShortFile(System.getProperty("user.dir") + "/output/" + qtcDiagOrderedFileName)));
		
		System.out.println("Entropy Encode QTC");
		byte[] encoded = EntropyEncDec.expGolombEnc(qtcRle);
		FileHelper.writeToFile(encoded, qtcEntropyEncodedFileName);
		
		// For VBS, just entropy encode, check if file present because feature may be off and no file is there
		if (FileHelper.fileExist(System.getProperty("user.dir") + "/output/" + vbsFileName)) {
			byte[] vbs = FileHelper.readByteFile(System.getProperty("user.dir") + "/output/" + vbsFileName);
			FileHelper.writeToFile(EntropyEncDec.expGolombEnc(vbs), vbsEntropyEncodedFileName);
		}

		if (FileHelper.fileExist(System.getProperty("user.dir") + "/output/" + qpFileName)) {
			byte[] qp = FileHelper.readByteFile(System.getProperty("user.dir") + "/output/" + qpFileName);
			FileHelper.writeToFile(EntropyEncDec.expGolombEnc(qp), qpEntropyEncodedFileName);
		}
		
		if (FileHelper.fileExist(System.getProperty("user.dir") + "/output/" + scFileName)) {
			byte[] sc = FileHelper.readByteFile(System.getProperty("user.dir") + "/output/" + scFileName);
			FileHelper.writeToFile(EntropyEncDec.expGolombEnc(sc), scEntropyEncodedFileName);
		}
	}
	
	/**
	 * Called by decoder before starting to decode. Assumes that all input files are ready.
	 */
	public static void generateAllEntropyDecodedFiles() {
		PropertyHelper ph = new PropertyHelper();
		String mvEntropyEncodedFileName = ph.getProperty("mvEntropyEncodedFileName");
		String mvEntropyDecodedFileName = ph.getProperty("mvEntropyDecodedFileName");
		String qtcEntropyEncodedFileName = ph.getProperty("qtcEntropyEncodedFileName");
		String qtcEntropyDecodedFileName = ph.getProperty("qtcEntropyDecodedFileName");
		String vbsEntropyEncodedFileName = ph.getProperty("vbsEntropyEncodedFileName");
		String vbsEntropyDecodedFileName = ph.getProperty("vbsEntropyDecodedFileName");
		String qpEntropyEncodedFileName = ph.getProperty("qpEntropyEncodedFileName");
		String qpEntropyDecodedFileName = ph.getProperty("qpEntropyDecodedFileName");
		String scEntropyEncodedFileName = ph.getProperty("scEntropyEncodedFileName");
		String scEntropyDecodedFileName = ph.getProperty("scEntropyDecodedFileName");
		int frameLength = Integer.valueOf(ph.getProperty("frameCount"));
        ...
        FileHelper.writeToFile(Helper.intArrToShortArr(mvEntropyDecoded), mvEntropyDecodedFileName); // converts int[] decoded mv to byte[]
        ...
        int[] qtc = new int[width*height*frameLength];
        ...
        FileHelper.writeToFile(Helper.intArrToShortArr(qtc), qtcEntropyDecodedFileName);
        
        // Reverse entropy encode vbs bytes,  if the encoded file exists
        if (FileHelper.fileExist(System.getProperty("user.dir") + "/output/" + vbsEntropyEncodedFileName)) {
        	byte[] vbs = FileHelper.readByteFile(System.getProperty("user.dir") + "/output/" + vbsEntropyEncodedFileName);
			FileHelper.writeToFile(Helper.intArrToByteArr(EntropyEncDec.expGolombDec(vbs)), vbsEntropyDecodedFileName);
		}
		
		if (FileHelper.fileExist(System.getProperty("user.dir") + "/output/" + qpEntropyEncodedFileName)) {
        	byte[] qp = FileHelper.readByteFile(System.getProperty("user.dir") + "/output/" + qpEntropyEncodedFileName);
			FileHelper.writeToFile(Helper.intArrToByteArr(EntropyEncDec.expGolombDec(qp)), qpEntropyDecodedFileName);
        }

		if (FileHelper.fileExist(System.getProperty("user.dir") + "/output/" + scEntropyEncodedFileName)) {
        	byte[] sc = FileHelper.readByteFile(System.getProperty("user.dir") + "/output/" + scEntropyEncodedFileName);
			FileHelper.writeToFile(Helper.intArrToByteArr(EntropyEncDec.expGolombDec(sc)), scEntropyDecodedFileName);
        }
	}
