...
public class FileHelper {
	 ...
	 * @param vbsSplit
	 */
	public static void writeToVBSFile(byte[] vbsSplit, int frameIndex) {
		PropertyHelper ph = new PropertyHelper();
		if (frameIndex == 0) {
			FileHelper.writeToFile(vbsSplit, ph.getProperty("vbsFileName"));
		}
		else {
			FileHelper.writeToFileAppend(vbsSplit, ph.getProperty("vbsFileName"));
		}
	}
	
	public static void writeToMVFile(ArrayList<Integer> mv, int frameIndex) {
		PropertyHelper ph = new PropertyHelper();
		if (frameIndex == 0) {
			FileHelper.writeToFile(Helper.intArrListToShortArr(mv), ph.getProperty("mvFileName"));
		}
		else {
			FileHelper.writeToFileAppend(Helper.intArrListToShortArr(mv), ph.getProperty("mvFileName"));
		}
	}

	public static void writeToReconstructedFile(byte[] reconstructedFrame, int frameIndex) {
		PropertyHelper ph = new PropertyHelper();
		if (frameIndex == 0)
			FileHelper.writeToFile(reconstructedFrame,  ph.getProperty("encReconstructedFileName"));
		else
			FileHelper.writeToFileAppend(reconstructedFrame,  ph.getProperty("encReconstructedFileName"));
	}
	 ...
	 * @param qtcEnc
	 */
	public static void writeToQtcDiagOrderedFile(short[] qtc, int frameIndex) {
		PropertyHelper ph = new PropertyHelper();
		if (frameIndex == 0)
			FileHelper.writeToFile(qtc,  ph.getProperty("qtcDiagOrderedFileName"));
		else
			FileHelper.writeToFileAppend(qtc,  ph.getProperty("qtcDiagOrderedFileName"));
	}
	 ...
	 * @param qtcEnc
	 */
	public static void writeToQtcFile(short[] qtc, int frameIndex) {
		PropertyHelper ph = new PropertyHelper();
		if (frameIndex == 0)
			FileHelper.writeToFile(qtc,  ph.getProperty("qtcFileName"));
		else
			FileHelper.writeToFileAppend(qtc,  ph.getProperty("qtcFileName"));
	}
