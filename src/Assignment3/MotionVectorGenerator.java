...
public class MotionVectorGenerator {
	 ...
	 * @return
	 */
	public int[] generateMVbyBlockFast(byte[] currFrame, ArrayList<byte[][]> buffers, int h1, int w1, int bs, int[] prevMV, int pass) {
			...
			*/
			...
			for (int i =0; i < 5; i++) {
				...
				if (w2 < 0 || w2+bs >= width_buffer || h2 < 0 || h2+bs >= height_buffer ||
					(w2 - 2*bs*w1) > 16 || (w2 - 2*bs*w1) < -16 || (h2 - 2*bs*h1) > 16 ||
					(h2 - 2*bs*h1) < 16)
					continue;

				PropertyHelper ph = new PropertyHelper();
				if (Integer.valueOf(ph.getProperty("RCflag")) == 3 && pass == 2) {
					if ((w2 - 2*bs*w1) > 1 || (w2 - 2*bs*w1) < -1 || (h2 - 2*bs*h1) > 1 ||
					(h2 - 2*bs*h1) < 1)
						continue;
				}
	 ...
	 * @return
	 */
	public int[] generateMVbyBlockFast(byte[] currFrame, byte[][] reference, int h1, int w1, int bs, int[] prevMV, int pass) {
		int minSAD = Integer.MAX_VALUE, minx = Integer.MAX_VALUE, miny = Integer.MAX_VALUE, minf = Integer.MAX_VALUE;
		boolean done = false;
		...
		while (!done) {
			...
			*/
			...
			for (int i =0; i < 5; i++) {
				...
				// out of bound, ignore
				if (w2 < 0 || w2+bs >= width || h2 < 0 || h2+bs >= height || w2-w1 < -16 || w2-w1 > 16 || h2-h1 < -16 || h2-h1 > 16)
					continue;

				PropertyHelper ph = new PropertyHelper();
				if (Integer.valueOf(ph.getProperty("RCflag")) == 3 && pass == 2) {
					if (w2-w1 < -1 || w2-w1 > 1 || h2-h1 < -1 || h2-h1 > 1)
						continue;
				}
